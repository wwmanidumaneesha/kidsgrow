...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['child_id']) && isset($_POST['health_division'])) {
    ...
    $family_health_division = $_POST['family_health_division'] ?? null;
    $supplementary_record   = $_POST['supplementary_record'] ?? null;
    $gn_record              = $_POST['gn_record'] ?? null;
    $weight                 = $_POST['weight'] ?? null;
    $recorded_date          = $_POST['recorded_date'] ?? date('Y-m-d'); // fallback to today if not provided

    // Ensure booleans are properly formatted for PostgreSQL
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            SET
                ...
                family_health_medical_officer_division = NULLIF(:family_health_division, ''),
                supplementary_record_number = NULLIF(:supplementary_record, ''),
                gn_record_number = NULLIF(:gn_record, ''),
                breastfeeding_only_2m = :breast2::BOOLEAN,
                breastfeeding_only_4m = :breast4::BOOLEAN
            WHERE child_id = :child_id
        ");
        $stmt->execute([
            ':health_division'        => $health_division,
            ':family_health_division' => $family_health_division,
        ...
        // 2) Insert/update weight record if valid
        if (!empty($weight) && is_numeric($weight)) {
            $weight = floatval($weight);

            // Validate recorded_date as YYYY-MM-DD
            $dateCheck = DateTime::createFromFormat('Y-m-d', $recorded_date);
            if (!$dateCheck) {
                throw new Exception("Invalid recorded date format. Expected YYYY-MM-DD.");
            }

            $stmt = $pdo->prepare("
                INSERT INTO child_weight_history (child_id, weight, recorded_date)
                VALUES (:child_id, :weight, :recorded_date)
                ON CONFLICT (child_id, recorded_date)
        ...
        // Commit
        $pdo->commit();
        echo json_encode(["status" => "success", "message" => "Child record and weight updated successfully"]);
        ...
        // Catch non-PDO exceptions (e.g. invalid date)
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
        }
        error_log("Validation error: " . $ex->getMessage());
        echo json_encode(["status" => "error", "message" => "Error updating record: " . $ex->getMessage()]);
    }
    exit;
}
...
<div class="modal" id="editModal">
    <h2>Update Child</h2>
    <form id="editForm">
        <!-- Ensure child_id is included in POST -->
        <input type="hidden" id="editChildId" name="child_id">

        <div class="form-group">
            <label>Child Name</label>
            <!-- Add name="child_name" so it’s posted -->
            <input type="text" class="form-control" id="editChildName" name="child_name" readonly>
        </div>
        ...
        <div class="form-group">
            ...
            <!-- Add name so it’s posted -->
            <input
                type="text"
                class="form-control"
                id="editHealthDivision"
                name="health_division"
            >
        </div>
        ...
        <div class="form-group">
            ...
            <input
                type="text"
                class="form-control"
                id="editFamilyHealthDivision"
                name="family_health_division"
            >
        </div>
        ...
        <div class="form-group">
            ...
            <input
                type="text"
                class="form-control"
                id="editSupplementaryRecord"
                name="supplementary_record"
            >
        </div>
  ...
  <script>
    $(document).ready(function(){
      ...
      $(document).on("click", ".edit-btn", function(){
          ...
          let weight = row.children().eq(8).text().trim(); // Assuming weight is in the 9th column
          ...
          $("#editRecordedDate").val(new Date().toISOString().split("T")[0]); // Default to today's date
      ...
      $(document).on("click", ".view-btn", function(){
          ...
          let gender = row.children().eq(9).text().trim();
          let supplementary = row.children().eq(10).text().trim();
          let gnRecord = row.children().eq(11).text().trim();
    ...
    // Open Modal for Editing with additional weight & recorded date fields
    $(document).on("click", ".edit-btn", function () {
        let row = $(this).closest("tr");
        let childId = row.data("id");
        let childName = row.children().eq(1).text().trim();
        ...
        $("#editGnRecord").val(gnRecord);
        $("#editWeight").val(weight);
        $("#editRecordedDate").val(new Date().toISOString().split("T")[0]); // Set today's date
    ...
    // Submit Edit Form with new weight logic
    $("#editForm").submit(function (e) {
        e.preventDefault();
        ...
        let recordedDate = $("#editRecordedDate").val();
    ...
    // Auto-Update Weight in Child Weight History
    $("#editWeight").on("input", function () {
        let weight = parseFloat($(this).val());
        if (weight < 0.5 || weight > 4.5) {
            showAlert("error", "Weight must be between 0.5 and 4.5 kg.");
            $(this).val("");
        }
    });

    // Auto-Update Recorded Date for Weight History
    $("#editRecordedDate").on("change", function () {
        let today = new Date().toISOString().split("T")[0];
        let selectedDate = $(this).val();

        if (selectedDate > today) {
            showAlert("error", "Recorded date cannot be in the future.");
            $(this).val(today);
        }
    });
});
