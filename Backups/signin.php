  ...
  <style>
    ...
    * {
    ...
    .left-section::before {
      ...
      right: 0;
      bottom: 0;
      /* Uncomment and update the URL below if you wish to use an illustration image */
...
<body>
  <div class="container">
    ...
    <div class="right-section">
      <div class="form-container">
        <!-- All sections share the same container -->
        <!-- Sign In Section -->
        <div id="loginSection">
        ...
        <!-- Forgot Password: Step 1 - Send OTP (Corrected) -->
  ...
  <script>
    // Toggle password visibility (login and reset)
    function togglePasswordVisibility(inputId = 'password', btn = null) {
    ...
    // Switch between sections (all sections are inside the same container)
      ...
      .then(data => {
        if (data.success) {
          ...
          if(data.role === 'Parent'){
            ...
            window.location.href = 'child_profile.php';
          }
        } else {
    ...
    // Attach keydown listener to the entire login form
    ...
    // Log the final POST body
    console.log("POST body:", formData.toString());
    
    // Make sure the URL is exactly "sendotp.php" (without an underscore)
    fetch('sendotp', {
      method: 'POST',
    ...
    .then(data => {
      ...
      if (data.success) {
        showPopup(data.message, 'success');
        // Auto-fill OTP email for convenience
    ...
    // Reset password via user_auth.php
