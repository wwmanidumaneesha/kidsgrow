...
// Clear the messages and user data from the session
unset($_SESSION['error_message'], $_SESSION['success_message'], $_SESSION['user_email'], $_SESSION['user_name']);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Ministry of Health - Sign Up</title>
    <style>
        body {
            margin: 0;
        ...
        background: none;
        padding: 0;
        width: 30px; /* Limit the button's width */
        height: 30px; /* Limit the button's height */
        display: flex;
        align-items: center;
        justify-content: center;
        }
...
<body>
    <div class="container">
        <div class="left-section">
            ...
            <img src="https://i.ibb.co/yYpj21s/image.png" alt="Doctor Illustration">
            <p>Copyright &copy; 2025 MSBK Software Solutions. All Rights Reserved.</p>
        </div>
        <div class="right-section">
            <div class="form-container">
                <div class="tabs">
                    <a id="sign-up-tab" class="active" onclick="toggleForm('sign-up')">Sign Up</a>
                    <a id="sign-in-tab" onclick="toggleForm('sign-in')">Sign In</a>
                </div>
                <form id="sign-up-form" method="post" action="user_auth.php">
                    <input type="hidden" name="action" value="sign_up">
                    <div class="form-group">
                        <label for="fullname">Full Name</label>
                        <input type="text" id="fullname" name="full_name" placeholder="Manidu Maneesha">
                    </div>
                    <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" id="email" name="email" placeholder="manidumaneeshaww@gmail.com">
                    </div>
                    <div class="form-group password-container">
                        <label for="password">Password</label>
                        <input type="password" id="password" name="password" placeholder="********">
                        <button type="button" class="toggle-password" onclick="togglePasswordVisibility(this, 'password')">
                            <i class="fas fa-eye"></i>
                        </button>
                    </div>
                    <button type="submit">Sign Up</button>
                    <div class="form-footer">
                        <a onclick="toggleForm('sign-in')">Already have an account?</a>
                    </div>
                </form>
                <form id="sign-in-form" class="hidden" method="post" action="user_auth.php">
                    <input type="hidden" name="action" value="sign_in">
                    <div class="form-group">
                        <label for="sign-in-email">Email</label>
                        <input type="email" id="sign-in-email" name="email" placeholder="Enter your email">
                    </div>
                    <div class="form-group password-container">
                        <label for="sign-in-password">Password</label>
                        <input type="password" id="sign-in-password" name="password" placeholder="Enter your password">
                        <button type="button" class="toggle-password" onclick="togglePasswordVisibility(this, 'sign-in-password')">
                            <i class="fas fa-eye"></i>
                        </button>
                    </div>
                    <button type="submit">Sign In</button>
                    <div class="form-footer">
                        <a onclick="toggleForm('sign-up')">Don't have an account?</a>
                    </div>
                </form>
            </div>
        </div>
    </div>
    <div id="errorPopup" class="error-popup <?php echo $errorMessage ? 'show' : ''; ?>">
        <?php echo htmlspecialchars($errorMessage); ?>
    </div>
    <!-- Success popup -->
    <div id="successPopup" class="success-popup <?php echo $successMessage ? 'show' : ''; ?>">
